...
class IORedirector:
    ...
    def __init__(self, name, original, new_redirect, wrap_buffer=False):
        ...
        if wrap_buffer and hasattr(original, "buffer"):
            self.buffer = IORedirector(name, original.buffer, new_redirect.buffer, False)
...
class CustomWriter(object):
    def __init__(self, name, wrap_stream, wrap_buffer, on_write=None):
        ...
        if wrap_buffer:
            self.buffer = CustomWriter(name, wrap_stream, wrap_buffer=False, on_write=on_write)
...
def redirect_output(stdout_handler, stderr_handler):
    ...
    _stdout_redirector = sys.stdout = IORedirector("stdout", sys.stdout, sys._vsc_out_buffer_, True)

    sys._vsc_err_buffer_ = CustomWriter("stderr", sys.stderr, True, stderr_handler)
    sys.stderr_original = sys.stderr
    _stderr_redirector = sys.stderr = IORedirector("stderr", sys.stderr, sys._vsc_err_buffer_, True)
