...
def _VSCODE_getVariableInfo(var):
    ...
    if hasattr(var, "shape"):
        try:
            ...
            if isinstance(var.shape, tuple) or typeName is not None and typeName == "EagerTensor":
...
def _VSCODE_getVariableProperties(var, listOfAttributes):
    result = {attr: repr(getattr(var, attr)) for attr in listOfAttributes if hasattr(var, attr)}
    return _VSCODE_json.dumps(result)
