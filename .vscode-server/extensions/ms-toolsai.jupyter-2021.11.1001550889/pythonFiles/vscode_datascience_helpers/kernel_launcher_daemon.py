...
from vscode_datascience_helpers.daemon.daemon_python import PythonDaemon as BasePythonDaemon
from vscode_datascience_helpers.daemon.daemon_python import change_exec_context, error_decorator
...
class PythonDaemon(JupyterDaemon):
    ...
    def m_prewarm_kernel(self):
        ...
        kernel_prewarm_starter = os.path.join(os.path.dirname(__file__), "kernel_prewarm_starter.py")
    ...
    def m_start_prewarmed_kernel(self, args=[]):
        ...
        def start_kernel():
            self.kernel.stdin.write("{0}{1}".format(json.dumps(args), os.linesep).encode("utf-8"))
    ...
    def _monitor_kernel(self):
        ...
        sys.stdout.flush()
        sys.stderr.flush()
        self._endpoint.notify("kernel_died", {"exit_code": exit_code, "reason": std_err})

    def _exec_module_observable_in_background(self, module_name, args=None, cwd=None, env=None):
        self.log.info(
